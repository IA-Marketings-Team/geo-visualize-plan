...
const Navbar: React.FC = () => {
  ...
  className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out ${
    isScrolled
      ? ' md:py-3 bg-black/80 backdrop-blur-sm shadow-md'
      : 'py-4 md:py-6 bg-transparent'
  }`}
>
  <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
    <Link to="/">
      <Logo size="md" className={isScrolled ? "text-[10px] md:text-base text-foreground" : "text-sm md:text-base"} />
    </Link>
        ...
        <nav className="container mx-auto py-4 px-4 flex flex-col space-y-3">
          {navItems.map((item, i) => (
            <Link
              key={item.name}
              to={item.path}
              className={`text-foreground text-base font-medium hover:text-geoplan-red transition-colors duration-200 ${
